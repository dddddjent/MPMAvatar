...
def render_predictions(root: Path, evaluation: Path, skip_video: bool) -> None:
    """Render the same predicted geometry used by the fitted Gaussian branch."""
    ...
    first = render.scientific_points_to_blender(vertices[0])
    objects = {}
    ...
    for frame, prediction in zip(frames, vertices, strict=True):
        render.update_vertices(objects, render.scientific_points_to_blender(prediction))
